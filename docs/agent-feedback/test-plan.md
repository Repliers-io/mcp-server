...
| A2 | Conditional instructions | `initialize` in both env states, read `result.instructions` | Both: the "Required sequence" line leads and `appliedFilters` appears inside the first 512 chars; no persona sentence, no tool-family list. With keys: 4 rules, rule 3 = "Report" and mentions send-feedback. Without: zero occurrences of "send-feedback", 3 rules, rule 3 = "_feedback blocks…" |
