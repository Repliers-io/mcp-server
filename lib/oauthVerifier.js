...
/**
 * This runs on the first request of a cutover, when the person reading the log has several new
 * secrets in play and no way to tell which one a bare number refers to. 401/403 is our own
 * introspection credential; 404 is the endpoint, which usually means MCP Auth is not enabled.
 */
function explainIntrospection(status, endpoint) {
  if (status === 401 || status === 403) {
    return (
      `Introspection at ${endpoint} refused our credentials (${status}). Check ` +
      `PROPELAUTH_MCP_INTROSPECT_CLIENT_ID and _SECRET: they come from PropelAuth's ` +
      `MCP -> Request Validation section and belong to one environment, which must be the ` +
      `environment OAUTH_BASE_URL points at.`
    );
  }
  if (status === 404) {
    return (
      `No introspection endpoint at ${endpoint} (404). Either MCP Auth is not enabled on this ` +
      `PropelAuth tenant, or OAUTH_INTROSPECTION_ENDPOINT / OAUTH_BASE_URL is wrong. See ` +
      `docs/oauth21/design.md section 6.`
    );
  }
  if (status >= 500) {
    return `PropelAuth's introspection endpoint is failing: ${endpoint} returned ${status}.`;
  }
  return `Introspection at ${endpoint} returned ${status}.`;
}
...
export function createIntrospectionVerifier({
  ...
  async function introspect(token) {
    ...
    if (!response.ok) {
      throw new ServerError(explainIntrospection(response.status, introspectionEndpoint));
    }
  ...
  async function introspectAndValidate(token, key) {
    ...
    if (!matched) {
      ...
      if (requireAudience) {
        // The client only learns that its token is wrong for us. The operator needs both sides of
        // the comparison: a client connecting on a different host than MCP_PUBLIC_URL declares
        // fails exactly here, after a login that looked successful.
        console.error(
          `[ERROR] Audience mismatch: token names ${named}, this server accepts ` +
            `${[...audiences].join(', ')} (from MCP_PUBLIC_URL).`
        );
